...
function error-message {
	sysconfig.sh 2> >(logger -t $(basename "$0") -i -p user.warning)
...
function cleanup {
if trap cleanup SIGINT ; then
 rm /tmp/sysinfo.$$
 rm /tmp/memoryinfo.$$
 rm /tmp/businfo.$$
 rm /tmp/cpuinfo.$$
 logger -t `basename "$0"` -i -p user.info -s “you used ^C cleaning Tmp files”
 exit 1
fi
}
